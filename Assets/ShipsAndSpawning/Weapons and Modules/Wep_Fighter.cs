...
public class Wep_Fighter : Weapon
{
        ...
        if (!reloading)
        {
            if (!(shotsLeft <= 0))
            {
                if (!onCoolDown)
                {
                    ...
                    {
                        Projectile projectile = Pooler.Instance.GetPooledGameObject(PooledObjectType.FighterBullet)?.GetComponent<Projectile>() ?? InstantiateProjectileHere(ToInstantiate);
                    ...
                    Projectile projectile_ = Pooler.Instance.GetPooledGameObject(PooledObjectType.FighterBullet)?.GetComponent<Projectile>() ?? InstantiateProjectileHere(ToInstantiate);
